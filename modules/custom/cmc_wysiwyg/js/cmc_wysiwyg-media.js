...
// COPIED FROM media/js/plugins/wysiwyg-media.js
// Changes in insertMediaFile()  
Drupal.cmcmedia = Drupal.cmcmedia || {};

// Define the behavior.
Drupal.wysiwyg.plugins.cmcmedia = {
  ...
  invoke: function (data, settings, instanceId) {
    //if (data.format == 'html') {
      Drupal.media.popups.mediaBrowser(function (mediaFiles) {        
        Drupal.wysiwyg.plugins.cmcmedia.mediaBrowserOnSelect(mediaFiles, instanceId);
      }, settings['global']);
    //}
  },
  ...
  mediaBrowserOnSelect: function (mediaFiles, instanceId) {
    ...
    Drupal.media.popups.mediaStyleSelector(mediaFile, function (formattedMedia) {
      Drupal.wysiwyg.plugins.cmcmedia.insertMediaFile(mediaFile, formattedMedia.type, formattedMedia.html, formattedMedia.options, Drupal.wysiwyg.instances[instanceId]);      
    }, options);
    
    return;
  },
  // CHANGES HERE FOR THE INSERTION
  insertMediaFile: function (mediaFile, viewMode, formattedMedia, options, wysiwygInstance) {
    ...
    if (mediaFile.type == 'image') {
      ...
      var inlineTag = Drupal.wysiwyg.plugins.cmcmedia.createTag(imgElement);
      inlineTag = inlineTag;
    }
    else {
      tagContent = {
        ...
        "view_mode": viewMode,
        "fid" : mediaFile.fid,
        "attributes": {'alt':'','class': viewMode}
      };
      
      inlineTag = '[[' + JSON.stringify(tagContent) + ']]';      
      
      if (mediaFile.type != 'video') {
        // link to pdf or something
        var classname = '';
        if (options.link_class) {
          classname = ' class="' + options.link_class + '" ';
        }
        inlineTag = '<a ' + classname + ' href="' + inlineTag + '">LINK TO ' + mediaFile.filename + '</a>';
      }
      //var inlineTag = Drupal.wysiwyg.plugins.cmcmedia.createTag(imgElement);    }
    }
    
    $.markItUp({ openWith: inlineTag} );    
  },
